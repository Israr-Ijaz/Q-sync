...
const features = [
    { title: 'Self-Driving Queue Engine', description: 'Patients scan a QR standee and track their live token on their phones with haptic alerts. Zero receptionist data entry required.', icon: Clock3, className: 'md:col-span-2 md:row-span-2', visual: 'queue' },
    { title: "The 'Double-Lock' Anti-Spam System", description: "Protect your clinic. Unverified online or cash bookings are held in a 'Pending' purgatory until visually verified by staff, keeping your active queue 100% accurate.", icon: MessageCircle, className: 'md:col-span-2', visual: 'whatsapp' },
    { title: 'Fail-Safe Tab Recovery', description: 'If a patient accidentally closes their browser, scanning the QR code again instantly restores their live tracking session. No lost tickets.', icon: FileText, className: 'md:col-span-1', visual: 'pdf' },
    { title: '1-Click WhatsApp Prescriptions', description: "Generate beautiful, print-ready A4 digital prescriptions and fire them directly to the patient's WhatsApp the moment the consultation ends.", icon: Users, className: 'md:col-span-1', visual: 'records' },
]

const plans = [
    { name: 'Manual Control', price: '$0', suffix: '', description: 'Everything you need to modernize your first patient flow.', features: ['Manual Receptionist Check-in', 'Basic Queue Dashboard', 'Standard PDF Prescriptions', 'Daily Token Reset'], cta: 'Start Free Trial', highlighted: false },
    { name: 'Autopilot', price: '$5', suffix: '/ month', description: 'The complete operating system for a clinic that moves fast.', features: ['Everything in Free', 'QR Self-Check-in & Live Sync', 'Double-Lock Anti-Spam', 'Tab Recovery System', '1-Click WhatsApp Rx'], cta: 'Upgrade to Pro', highlighted: true },
]

function Logo() {
    return <a href="#top" className="flex items-center gap-2.5 text-sm font-semibold tracking-tight text-foreground" aria-label="Opedox home"><Image src="/icon.png" alt="Opedox Logo" width={32} height={32} className="rounded-xl shadow-sm" /><span className="text-[17px]">opedox</span></a>
}
...
function CardVisual({ visual }: { visual: string }) {
    ...
    return <div className="h-44 w-64 rounded-tl-2xl border border-mint/20 bg-background p-4 shadow-[0_0_35px_var(--mint-glow)]"><div className="mb-4 flex justify-between text-[10px] text-subtle"><span>LIVE QUEUE</span><span className="text-mint">Online</span></div>{['Asif Arshad', 'israr ijaz', 'Aera Khan'].map((name, i) => <div key={name} className="mb-2 flex items-center gap-2 rounded-md border border-line bg-panel px-2 py-2 text-[10px] text-muted-foreground"><span className={`size-1.5 rounded-full ${i === 0 ? 'bg-mint' : 'bg-subtle'}`} />{name}<span className="ml-auto text-subtle">{i + 1} min</span></div>)}</div>
}
    ...
    return <main id="top" className="min-h-screen overflow-hidden bg-background text-foreground">
        ...
        <nav className="relative z-10 mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-8">
            ...
            <div className="hidden items-center gap-5 md:flex">
                <a href="/login" className="text-sm font-medium text-slate-400 transition-colors hover:text-white">Login</a>
                <a href="/signup" className="text-sm font-medium text-slate-400 transition-colors hover:text-white">Sign Up</a>
                <a href="/login" className="rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-bold text-white transition-all shadow-[0_4px_14px_rgba(16,185,129,0.3)] hover:bg-emerald-500 hover:-translate-y-0.5 hover:shadow-[0_6px_20px_rgba(16,185,129,0.4)]">Start Free Trial</a>
            </div>
            <button type="button" className="rounded-lg border border-line p-2 text-muted-foreground md:hidden" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu" aria-expanded={menuOpen}>{menuOpen ? <X size={19} /> : <Menu size={19} />}</button>
        </nav>
        {menuOpen && <div className="relative z-10 mx-6 flex flex-col gap-4 rounded-xl border border-line bg-panel p-5 text-sm text-muted-foreground md:hidden">
            <a href="#features" onClick={() => setMenuOpen(false)}>Features</a>
            <a href="#solutions" onClick={() => setMenuOpen(false)}>Solutions</a>
            <a href="#pricing" onClick={() => setMenuOpen(false)}>Pricing</a>
            <a href="/login" className="text-lg font-semibold text-slate-300 hover:text-white">Login</a>
            <a href="/signup" className="text-lg font-semibold text-slate-300 hover:text-white">Sign Up</a>
            <a href="/login" className="text-lg font-bold text-emerald-400">Start Free Trial</a>
        </div>}
        ...
        <section id="features" className="relative z-10 mx-auto max-w-7xl px-6 py-24 lg:px-8 lg:py-32">
            <div className="mb-12 max-w-xl">
                <p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-mint">Built for momentum</p>
                <h2 className="text-balance text-4xl font-semibold tracking-[-0.04em] sm:text-5xl">Every prescription, beautifully in motion.</h2>
                <p className="mt-5 text-base leading-7 text-muted-foreground">Opedox removes the friction between a clinical decision and a patient feeling taken care of.</p>
            </div>
            <div className="grid auto-rows-[230px] gap-4 md:grid-cols-4">
                {features.map(({ title, description, icon: Icon, className, visual }) => (
                    <article key={title} className={`group relative overflow-hidden rounded-2xl border border-line bg-panel p-6 transition-all hover:border-mint/40 hover:bg-panel-strong hover:shadow-[0_0_35px_var(--mint-glow)] ${className}`}>
                        <div className="relative z-10 flex h-full flex-col">
                            <span className="flex size-9 items-center justify-center rounded-lg border border-line bg-background text-mint"><Icon size={17} /></span>
                            <div className="mt-auto">
                                <h3 className={className.includes('row-span-2') ? "text-2xl font-black text-white tracking-tight" : "text-lg font-medium tracking-tight"}>{title}</h3>
                                <p className={className.includes('row-span-2') ? "mt-3 text-base text-slate-400 leading-relaxed" : "mt-2 max-w-sm text-sm leading-6 text-muted-foreground"}>{description}</p>
                            </div>
                        </div>
                        <div className="absolute bottom-[-3rem] right-[-1rem] opacity-80 transition-transform duration-500 group-hover:-translate-y-2" aria-hidden="true"><CardVisual visual={visual} /></div>
                    </article>
                ))}
            </div>
        </section>
        ...
        <section id="pricing" className="relative z-10 mx-auto max-w-5xl px-6 py-24 lg:py-32">
            <div className="mb-12 text-center">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-mint">Simple, transparent pricing</p>
                <h2 className="mt-4 text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">Start free. Scale with confidence.</h2>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
                {plans.map((plan) => (
                    <article key={plan.name} className={`rounded-2xl border p-7 ${plan.highlighted ? 'border-mint/50 bg-mint/[0.07] shadow-[0_0_50px_var(--mint-glow)]' : 'border-line bg-panel'}`}>
                        <div className="flex items-start justify-between">
                            <div>
                                <h3 className="text-lg font-medium">{plan.name}</h3>
                                <p className="mt-2 max-w-xs text-sm leading-6 text-muted-foreground">{plan.description}</p>
                            </div>
                            {plan.highlighted && <span className="rounded-full bg-mint px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-background">Popular</span>}
                        </div>
                        ...
                        <a href="/login" className={`mt-8 flex h-11 items-center justify-center rounded-lg text-sm font-medium shadow-[0_0_24px_var(--mint-glow)] transition-transform hover:-translate-y-0.5 ${plan.highlighted ? 'bg-mint text-background' : 'border border-line bg-background text-foreground'}`}>
                            {plan.cta}
                        </a>
                    </article>
                ))}
